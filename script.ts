...
(function () {
  ...
  function calcTempo(mil: number) {
    const dia = Math.floor(mil / 86400000);
    const hr = Math.floor((mil % 86400000) / 3600000);
    const min = Math.floor((mil % 3600000) / 60000);
    const seg = Math.floor((mil % 60000) / 1000);

    let tempoPatio: string;
    if (Math.floor(dia) > 0) {
      tempoPatio = `${dia}d ${hr}h ${min}m e ${seg}s`;
    } else if (Math.floor(hr) > 0) {
      tempoPatio = `${hr}h ${min}m e ${seg}s`;
    } else {
      tempoPatio = `${min}m e ${seg}s`;
    }

    //const min = Math.floor(mil / 60000);
    //const seg = Math.floor((mil % 60000) / 1000);
    return tempoPatio;
    //return `${hr}h ${min}m e ${seg}s`;
